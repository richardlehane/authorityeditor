...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="camel_case">
        ...
        <xsl:variable name="first_up"
                      select="translate($first_letter, $lowerCaseChars, $upperCaseChars)" />
                    ...
                    <xsl:when test="$classes[1]/rda:Disposal/rda:DisposalCondition='Authorised'">
                        <xsl:call-template name="testcustody">
                            <xsl:with-param name="disposals"
                                            select="$classes[1]/rda:Disposal[rda:DisposalCondition='Authorised']" />
                            <xsl:with-param name="classes"
                                            select="$classes[position() > 1]" />
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:call-template name="testcustody">
                            <xsl:with-param name="disposals"
                                            select="$classes[1]/rda:Disposal[not(rda:DisposalCondition='Automated')]" />
                            <xsl:with-param name="classes"
                                            select="$classes[position() > 1]" />
                        </xsl:call-template>
                    </xsl:otherwise>
