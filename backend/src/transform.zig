...
fn insertTransformed(doc: *Document, stylesheet_path: []const u8, output_file: []const u8, output_path: []const u8) void {
...
//     defer doc.deinit();
//     xml.exsltCommonRegister();
//     insertTransformed(doc, "../data/stylesheets/xslt/draft.xsl", "draft.docx", "word/document.xml");
//     insertTransformed(doc, "../data/stylesheets/xslt/draft_header_two.xsl", "draft.docx", "word/header2.xml");
//     insertTransformed(doc, "../data/stylesheets/xslt/draft_header_three.xsl", "draft.docx", "word/header3.xml");
// }

test "approved" {
    const out = "approved.docx";
    std.fs.cwd().deleteFile(out) catch |e| {
        try testing.expectEqual(e, error.FileNotFound);
    };
    var dir = try std.fs.cwd().openDir("../data/stylesheets/Approved/Approved Authority", .{ .iterate = true });
    defer dir.close();
    var walker = try dir.walk(testing.allocator);
    defer walker.deinit();
    while (try walker.next()) |entry| {
        if (entry.kind == .file) {
            const buf = try dir.readFileAlloc(testing.allocator, entry.path, 100_000_000);
            const status = miniz.mz_zip_add_mem_to_archive_file_in_place(out, entry.path, @ptrCast(buf), buf.len, null, 0, miniz.MZ_BEST_COMPRESSION);
            try testing.expect(status == 1);
            testing.allocator.free(buf);
        }
    }
...
test "appraisal" {
    const out = "appraisal_report.docx";
    std.fs.cwd().deleteFile(out) catch |e| {
        try testing.expectEqual(e, error.FileNotFound);
    };
    var dir = try std.fs.cwd().openDir("../data/stylesheets/Appraisal Report/Appraisal Report", .{ .iterate = true });
    defer dir.close();
    var walker = try dir.walk(testing.allocator);
    defer walker.deinit();
    while (try walker.next()) |entry| {
